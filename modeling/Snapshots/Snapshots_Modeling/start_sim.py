"""
This code has one main function:
...
def generate_all_snapshots(state_dict, main_dir, topol_dir, items_to_copy, job_template, number_of_runs):
    """
    NOTE: Before running generate_all_snapshots function, topol_dir should contain:
    -{state}_{time}_topol.txt topology files created with
    IMP.spatiotemporal.prepare_protein_library.prepare_protein_library
    And the main_dir should contain:
    -static_snapshot.py
    ...
    for time in state_dict.keys():
        for state in range(1,state_dict[time]+1):
            ...
            shutil.copy2(os.path.join(topol_dir, topol_file), new_dir)
