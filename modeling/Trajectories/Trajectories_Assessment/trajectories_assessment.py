...
-calculation of precision of the model (purity)
...
samples (SampleA and SampleB) obtained from each snapshot produced by the imp sampcon exhaust function during
 the snapshot assessment process. The similarity of these independent samples across all snapshots can be easily 
 verified in the KS-test.txt file.
...
# Output from sampling precision and purity to be saved in united dir: analysis_output
...
# 2 - calculation of precision of the model (purity)

# purity is calculated from .labeled_pdf.txt in Trajectories_Modeling dir
trajectories_modeling_input_dir = "../Trajectories_Modeling/output/"

analysis.purity(trajectories_modeling_input_dir+'labeled_pdf.txt', output_fn = analysis_output + 'purity.txt')

os.chdir(main_dir) # it is crucial that after each step, directory is changed back to main
print("Step 2: calculation of precision of the model (purity) IS COMPLETED")
print("")
print("")
...
def convert_rmfs(state_dict, model, custom_path=None):
    ...
    :return: ?? Idk how to add any kind of return here ??
    '''
...
def copy_dat_files(custom_src_dir = None):
    """
